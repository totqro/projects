...
function showTab(t) {
    ...
    else if (t==='parlays') { $('parlays-tab').style.display='block'; $$('.tab-button')[1].classList.add('active'); displayParlays(); }
...
// Parlays
function filterParlays(legs) {
    currentParlayFilter = legs;
    $$('.parlay-legs-btn').forEach(b => b.classList.remove('active'));
    document.querySelector(`.parlay-legs-btn[data-legs="${legs}"]`).classList.add('active');
    displayParlays();
}

function sortParlays(sortBy) {
    currentParlaySort = sortBy;
    displayParlays();
}

function displayParlays() {
    let parlays = [...allParlays];
    if (currentParlayFilter !== 'all') parlays = parlays.filter(p => p.n_legs === parseInt(currentParlayFilter));
    ...
    parlays.sort(sorts[currentParlaySort] || sorts.ev);

    if (!parlays.length) {
        $('parlay-count').textContent = '0';
        $('parlay-best-payout').textContent = '-';
        $('parlay-best-ev').textContent = '-';
        $('parlay-best-prob').textContent = '-';
        $('parlays-list').innerHTML = `<div class="no-data"><div class="no-data-icon">🎰</div><p>No parlay combinations available.</p><p style="font-size:.82rem;margin-top:6px;color:var(--t3)">Need at least 2 ML favorite picks from different games.</p></div>`;
    }

    // Always load parlay performance history
    loadParlayPerformance();

    if (!parlays.length) return;

    $('parlay-count').textContent = parlays.length;
    $('parlay-best-payout').textContent = usd(Math.max(...parlays.map(p => p.payout)));
    $('parlay-best-ev').textContent = signUsd(Math.max(...parlays.map(p => p.ev)));
    $('parlay-best-prob').textContent = pct(Math.max(...parlays.map(p => p.combined_true_prob)));

    $('parlays-list').innerHTML = parlays.map((p, i) => {
        const evClass = p.ev >= 0 ? 'positive' : 'negative';
        const legsHtml = p.legs.map(leg => {
            const gc = getGradeClass(getGrade(leg.edge));
            return `<div class="parlay-leg">
                <div class="parlay-leg-left">
                    <span class="grade ${gc}" style="font-size:.65rem;min-width:24px;padding:2px 6px">${getGrade(leg.edge)}</span>
                    <div class="parlay-leg-info">
                        <span class="parlay-leg-pick">${leg.pick}</span>
                        <span class="parlay-leg-game">${leg.game}</span>
                    </div>
                </div>
                <div class="parlay-leg-right">
                    <span class="parlay-leg-odds">${fmtOdds(leg.odds)}</span>
                    <span class="parlay-leg-prob">${pct(leg.true_prob,0)} prob</span>
                </div>
            </div>`;
        }).join('');
        ...
        return `<div class="parlay-card${isTop?' parlay-top':''}" onclick="this.classList.toggle('expanded')">
            <div class="parlay-header">
                <div class="parlay-header-left">
                    ${isTop?'<span class="parlay-top-badge">⭐ BEST</span>':''}
                    <span class="parlay-badge">${p.n_legs}-LEG</span>
                    <span class="parlay-odds">${fmtOdds(p.combined_odds)}</span>
                </div>
                <div class="parlay-header-right">
                    <div class="parlay-header-stat">
                        <span class="parlay-header-label">Payout</span>
                        <span class="parlay-header-value">${usd(p.payout)}</span>
                    </div>
                    <div class="parlay-header-stat">
                        <span class="parlay-header-label">EV</span>
                        <span class="parlay-header-value ${evClass}">${signUsd(p.ev)}</span>
                    </div>
                    <div class="parlay-header-stat">
                        <span class="parlay-header-label">Hit Rate</span>
                        <span class="parlay-header-value">${pct(p.combined_true_prob,1)}</span>
                    </div>
                </div>
            </div>
            <div class="parlay-body">
                ...
                <div class="parlay-summary">
                    <div class="parlay-summary-item"><span>Combined Edge</span><span class="edge-val">${pct(p.edge)}</span></div>
                    <div class="parlay-summary-item"><span>ROI</span><span class="${evClass}">${pct(p.roi)}</span></div>
                    <div class="parlay-summary-item"><span>Implied Prob</span><span>${pct(p.combined_implied_prob)}</span></div>
                    <div class="parlay-summary-item"><span>Stake</span><span>${usd(p.stake)}</span></div>
                </div>
            </div>
        </div>`;
    }).join('');
...
async function loadParlayPerformance() {
    try {
        ...
        displayParlayPerformance(await r.json());
...
function displayParlayPerformance(data) {
    ...
    // Show sections
    $('parlay-perf-section').style.display = '';
    $('parlay-history-section').style.display = '';

    // Summary stats
    const profitClass = data.total_profit >= 0 ? 'positive' : 'negative';
    $('parlay-perf-total').textContent = data.total_parlays;
    $('parlay-perf-wr').textContent = pct(data.win_rate);
    const roiEl = $('parlay-perf-roi');
    roiEl.textContent = pct(data.roi);
    roiEl.className = `stat-value ${profitClass}`;
    ...
    if (Object.keys(byLegs).length) {
        $('parlay-legs-breakdown').style.display = '';
        $('parlay-legs-stats').innerHTML = Object.entries(byLegs).map(([n, d]) => {
            const pc = d.profit >= 0 ? 'positive' : 'negative';
            return `<div class="grade-performance-item">
                <div class="grade-performance-badge grade-b-plus">${n}-Leg</div>
    ...
    // Recent results
    const recent = (data.parlays || []).slice(0, 20);
    $('parlay-history-list').innerHTML = recent.map(p => {
        const icon = p.result === 'won' ? '✅' : '❌';
        const legs = p.legs.map(l => l.pick).join(' + ');
        const legResults = p.legs.map(l => l.result === 'won' ? '✅' : '❌').join('');
        const d = new Date(p.date + 'T12:00:00');
        return `<div class="result-item" style="grid-template-columns:36px 1fr auto auto auto">
            <div class="result-icon">${icon}</div>
            <div class="result-details">
                <div class="result-pick">${legs}</div>
                <div class="result-game">${p.n_legs}-leg parlay · ${legResults}</div>
            </div>
            <div class="result-date">${d.toLocaleDateString('en-US',{month:'short',day:'numeric'})}</div>
            <div class="result-grade" style="min-width:60px;text-align:center">${fmtOdds(p.combined_odds)}</div>
