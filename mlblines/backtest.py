...
MLB Model Backtest
==================
Runs the prediction model retrospectively on completed MLB games to evaluate
accuracy on game winner and total runs predictions.

Usage:
    python backtest.py                    # Backtest since April 1
    python backtest.py --start 2026-03-20 # Custom start date
"""
...
from src.data.mlb_data import fetch_season_games, fetch_standings, get_team_recent_form
from src.models.model import find_similar_games, estimate_probabilities

EST = timezone(timedelta(hours=-4))


def run_backtest(start_date: str = "2026-04-01"):
    print(f"MLB Model Backtest — Games since {start_date}")
    print("=" * 60)
    ...
    print(f"  Avg total runs error: {avg_total_error:.2f} runs")
    print(f"  Total within 1 run:   {within_1:.1%}")
    print(f"  Total within 2 runs:  {within_2:.1%}")
    print("=" * 60)
    ...
    output = {
        ...
        "within_1_run": round(within_1, 4),
        "within_2_runs": round(within_2, 4),
        "results": sorted(results, key=lambda r: r["date"], reverse=True),
    }

    out_path = Path(__file__).parent / "mlbdata" / "backtest_results.json"
